...
int main(int, char**)
{
    ...
    ::RegisterClassEx(&wc);
    HWND hwnd = ::CreateWindowEx(WS_EX_LAYERED, wc.lpszClassName, _T("Dear ImGui DirectX9 Example window"), WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc.hInstance, NULL);
    ...
    while (!done)
    {
        ...
        ImGui::Begin("menu", 0, ImGuiWindowFlags_NoDecoration);
        {
            ...
            ImGui::BeginChild("main", { window->Size.x - 81, window->Size.y - window->DC.CursorPos.y + window->Pos.y }, 0, ImGuiWindowFlags_NoBackground);
            {
                ...
                case 0: {
                    ...
                    ImGui::BeginChild("main_child", { ImGui::GetWindowWidth(), ImGui::GetWindowHeight() - 55 }, 0, ImGuiWindowFlags_NoBackground);
                    {
                        ImGui::BeginGroup();
                        {
                            custom::begin_child("Window 1", { (ImGui::GetWindowWidth() - 30) / 2 - ImGui::GetStyle().ItemSpacing.x / 2, 300 });
                            {
                            ...
                            custom::begin_child("Window 3", { (ImGui::GetWindowWidth() - 30) / 2 - ImGui::GetStyle().ItemSpacing.x / 2, 300 });
                            {
                                static bool enabled = true;
                                static int slider = 0, combo = 0;
                                static char text[64];
                        ...
                        ImGui::BeginGroup();
                        {
                            custom::begin_child("Window 2", { (ImGui::GetWindowWidth() - 30) / 2 - ImGui::GetStyle().ItemSpacing.x / 2, 300 });
                            {
                                static bool enabled = true;
                                static int slider = 0, combo = 0;
                                static char text[64];
                            ...
                            custom::begin_child("Window 4", { (ImGui::GetWindowWidth() - 30) / 2 - ImGui::GetStyle().ItemSpacing.x / 2, 300 });
                            {
                                static bool enabled = true;
                                static int slider = 0, combo = 0;
                                static char text[64];
            ...
            ImGui::BeginChild("navbar", { 50 + 100 * navbar_width, window->Size.y }, 0, ImGuiWindowFlags_NoBackground);
            {
                ...
                if (custom::tab("C", "Miscs", tab == 2)) tab = 2;
                if (custom::tab("D", "Settings", tab == 3)) tab = 3;
                if (custom::tab("E", "Loads", tab == 4)) tab = 4;
                ImGui::PopStyleVar();
            }
            ImGui::EndChild();
